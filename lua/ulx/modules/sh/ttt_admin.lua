...
function ulx.force(calling_ply, target_plys, target_role, should_silent)
	if not GetConVarString("gamemode") == "terrortown" then ULib.tsayError(calling_ply, gamemode_error, true) else
		...
		local imp_starting_credits = GetConVarNumber("ttt_imp_credits_starting")
		...
		if target_role == "innocent" then role, role_grammar, role_string, role_credits = ROLE_INNOCENT, "an ", "innocent", 0 end
		if target_role == "traitor" then role, role_grammar, role_string, role_credits = ROLE_TRAITOR, "a ", "traitor", starting_credits end
		if target_role == "detective" then role, role_grammar, role_string, role_credits = ROLE_DETECTIVE, "a ", "detective", det_starting_credits end
		if target_role == "jester" then role, role_grammar, role_string, role_credits = ROLE_JESTER, "a ", "jester", 0 end
		if target_role == "swapper" then role, role_grammar, role_string, role_credits = ROLE_SWAPPER, "a ", "swapper", 0 end
		if target_role == "glitch" then role, role_grammar, role_string, role_credits = ROLE_GLITCH, "a ", "glitch", 0 end
		if target_role == "phantom" then role, role_grammar, role_string, role_credits = ROLE_PHANTOM, "a ", "phantom", 0 end
		if target_role == "hypnotist" then role, role_grammar, role_string, role_credits = ROLE_HYPNOTIST, "a ", "hypnotist", hyp_starting_credits end
		if target_role == "revenger" then role, role_grammar, role_string, role_credits = ROLE_REVENGER, "a ", "revenger", 0 end
		if target_role == "drunk" then role, role_grammar, role_string, role_credits = ROLE_DRUNK, "a ", "drunk", 0 end
		if target_role == "clown" then role, role_grammar, role_string, role_credits = ROLE_CLOWN, "a ", "clown", 0 end
		if target_role == "deputy" then role, role_grammar, role_string, role_credits = ROLE_DEPUTY, "a ", "deputy", 0 end
		if target_role == "impersonator" then role, role_grammar, role_string, role_credits = ROLE_IMPERSONATOR, "an ", "impersonator", imp_starting_credits end
		if target_role == "beggar" then role, role_grammar, role_string, role_credits = ROLE_BEGGAR, "a ", "beggar", 0 end
		if target_role == "old man" then role, role_grammar, role_string, role_credits = ROLE_OLDMAN, "an ", "old man", 0 end
